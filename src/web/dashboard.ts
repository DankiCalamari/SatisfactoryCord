...
export function dashboardHtml(snapshot: LifecycleSnapshot, discordConnected: boolean, capabilities: Capabilities): string {
  const uptime = snapshot.startedAt ? formatDuration((Date.now() - snapshot.startedAt.getTime()) / 1000) : "0s";
  const dot = (ok: boolean) => `<span class="${ok ? "ok" : "bad"}">${ok ? "Online" : "Unavailable"}</span>`;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
