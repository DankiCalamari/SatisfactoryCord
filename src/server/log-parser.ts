...
export function parseLogLine(rawLine: string, now = new Date()): ServerEvent[] {
  const line = sanitiseLogText(rawLine);
  if (!line) return [];
  const events: ServerEvent[] = [consoleEvent(line, now)];

  const chat = parseChat(line, now);
  if (chat) events.push(chat);

  const joined = firstMatch(line, joinPatterns);
  if (joined) events.push({ type: "player-joined", timestamp: now, playerName: joined });

  const left = firstMatch(line, leavePatterns);
  if (left) events.push({ type: "player-left", timestamp: now, playerName: left });
...
function parseChat(line: string, timestamp: Date): ServerEvent | undefined {
...
function consoleEvent(line: string, timestamp: Date): ServerEvent {
