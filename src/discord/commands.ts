...
export async function registerDiscordCommands(config: AppConfig, clientId: string): Promise<void> {
  if (!config.discord.enabled || !config.discord.token || !config.discord.guildId) return;
