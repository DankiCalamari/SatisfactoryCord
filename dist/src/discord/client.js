...
export async function createDiscordClient(config, logger, processManager, api, commandProvider, messageProvider, getCapabilities) {
    ...
    client.once("ready", async () => {
        ...
        if (client.user?.id)
            await registerDiscordCommands(config, client.user.id);
...
function setPresence(client, state, players) {
...
async function handleSlashCommand(interaction, config, logger, processManager, api, commandProvider, getCapabilities) {
    if (interaction.commandName !== "satisfactory")
        return;
...
async function handleDiscordMessage(message, config, provider) {
    if (message.author.bot || !config.relay.discordToGame || !provider.available)
        return;
    if (message.channelId !== config.discord.chatChannelId)
        return;
