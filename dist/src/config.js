...
export function loadConfig(configPath = "config.yml") {
    ...
    const workingDirectory = env("SATISFACTORY_WORKING_DIRECTORY", fileConfig.server?.workingDirectory ?? process.cwd());
    const cfg = {
        server: {
        ...
        moderation: {
            ...
            blockedWordsFile: env("CHAT_BLOCKED_WORDS_FILE", fileConfig.moderation?.blockedWordsFile ?? "config/blocked-words.txt"),
        ...
        inGameAdmin: {
            ...
            requireExactNameMatch: boolEnv("IN_GAME_ADMIN_REQUIRE_EXACT_NAME_MATCH", fileConfig.inGameAdmin?.requireExactNameMatch ?? true)
        },
        web: {
...
function defaultLogPath(workingDirectory) {
