...
export function sanitiseLogText(value) {
    return stripAnsi(value).replace(controlPattern, "").replace(/\r?\n/g, " ").trim();
}
export function sanitiseDiscordOutbound(value) {
    return sanitiseLogText(value).replace(mentionPattern, "@\u200b$1");
}
export function sanitiseGameMessage(author, message, maxLength = 400) {
...
export function assertSafeConsoleCommand(command, maxLength = 200) {
